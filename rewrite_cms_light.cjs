...
import { Save, Image as ImageIcon, LayoutTemplate, FileText, HelpCircle, CheckCircle, Upload, CheckCircle2, RefreshCcw, LogOut, AlertCircle } from 'lucide-react';
...
export function CMSDashboard({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] flex flex-col md:flex-row font-sans">
      {/* Light Sidebar */}
      <aside className="w-full md:w-72 bg-white border-r border-slate-200 shrink-0 md:min-h-screen flex flex-col z-20">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden bg-[#fafafa]">
        {/* Topbar */}
        <header className="bg-white/80 backdrop-blur-md px-8 py-5 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0 shadow-sm z-10 sticky top-0">
          <div>
